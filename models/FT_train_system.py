...
from models.inversion_model import InvEncoder, InvDecoder, Projector, get_hidden_size, InvSystem

class FT_DataCollator_System:
    def __call__(self, features, return_tensors=None):
        ...
        return {
            ...
            'encoder_input_attention_mask': torch.stack(encoder_input_attention_mask),
            'decoder_input_ids': torch.stack(decoder_input_ids),
            'decoder_input_attention_mask': torch.stack(decoder_input_attention_mask),
            'labels': torch.stack(labels),
            'labels_attention_mask': torch.stack(labels_attention_mask),
        }

class FT_Trainer_System(transformers.Trainer):
...
def local_alignment_system(args, data):
    ...
    labels = [labels[i] for i in range(0, len(labels), 8)]
    n = len(input_ids)
    ...
    res = t1(input_ids, return_tensors='pt', padding=True, truncation=True, max_length=256) # t5-base
    encoder_input_ids = res['input_ids']
    encoder_input_ids = encoder_input_ids.reshape(n//8, 8, encoder_input_ids.shape[1])
    print(encoder_input_ids.shape)
    encoder_input_attention_mask = res['attention_mask']
    encoder_input_attention_mask = encoder_input_attention_mask.reshape(n//8, 8, encoder_input_attention_mask.shape[1])

    res = t2(input_ids, return_tensors='pt', padding=True, truncation=True, max_length=256) # llama2
    decoder_input_ids = res['input_ids']
    decoder_input_ids = decoder_input_ids.reshape(n//8, 8, decoder_input_ids.shape[1])
    print(decoder_input_ids.shape)
    decoder_input_attention_mask = res['attention_mask']
    decoder_input_attention_mask = decoder_input_attention_mask.reshape(n//8, 8, decoder_input_attention_mask.shape[1])
    ...
    trainer = FT_Trainer_System(
        model=model,
        args=training_args,
        train_dataset=train_dataset,
        data_collator=FT_DataCollator_System(),
    )
    trainer.train()
...
def global_finetuning_system(args, data):
    encoder = InvEncoder(model_path=args.encoder_model_path)
    decoder = InvDecoder(model_path=args.decoder_model_path)
    encoder_hidden_size = get_hidden_size(args.encoder_model_name)
    decoder_hidden_size = get_hidden_size(args.decoder_model_name)
